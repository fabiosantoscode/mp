...
module.exports = function addSounds({ mp }) {
    ...
    mp.entities.on('add', (ent) => {
        ...
        if (ent instanceof mp.MortarBullet || ent instanceof mp.BazookaBullet) {
            ...
            var onceBump
            var solidsTouched = new WeakMap()
            ent.on('bump', onceBump = function onceBump({ collidingWithMe }) {
                var anySolid = false
                for (var cur = collidingWithMe; collidingWithMe.entity; collidingWithMe = collidingWithMe.next)
                    if (collidingWithMe.entity.solid && !solidsTouched.has(collidingWithMe.entity)) {
                        solidsTouched.set(collidingWithMe.entity)
                        anySolid = true
                        break;
                    }
                if (!anySolid) { return }
