...
module.exports = function makeEntityCollection() {
    ...
    collection.collidingWithNonStaticLL = function(entity) {
        if (qt) {
            if (arguments.length >= 4) {
                return qt.get_non_static_ll.apply(null, arguments)
            }
            // Cache cent, which is a computed prop
            var cent = entity.center
            return qt.get_non_static_ll(
                cent.x - (entity.size.x / 2),
                cent.y - (entity.size.y / 2),
                entity.size.x,
                entity.size.y,
    ...
    collection.push = function (item) {
        ...
        item.onUpdateCenter = (center, newCentX, newCentY) => {
            if (item._qtObj) {
                var centerChanged =
                    center.x !== newCentX
                 || center.y !== newCentY
                ...
                if (!sizeChanged) {
                    if (foundInside === 'c' && !isRoot) {
                        ...
                        if (node.x < newCentX - (w / 2) &&
                            node.x + node.w > newCentX + (w / 2) &&
                            node.y < newCentY - (h / 2) &&
                            node.y + node.h > newCentY + (h / 2)) {
                            // Skippin'!
                            item._qtObj.x = x
                            item._qtObj.y = y
                            item._qtObj.w = w
            ...
            item._qtObj.x = newCentX - (item.size.x / 2)
            item._qtObj.y = newCentY - (item.size.y / 2)
