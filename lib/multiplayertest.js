...
function gameLoop({ mp, main, camera, player, isServer }) {
    ...
    ;(function onFrame() {
        ...
        draw(extrapolation)
        if (isServer) main.networld.commit()
...
function makeServer() {
    ...
    gameLoop({ mp: mp, main: main, camera: camera, player: player })
    return main
}
...
function makeClient(canvas) {
    ...
    gameLoop({ mp: mp, main: main, camera: null, player: null })
