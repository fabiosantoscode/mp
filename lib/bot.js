...
module.exports = function makeBotSocket({ mp }) {
    ...
    function isUnderSights() {
        return !!enemy && !enemy.dead && enemy.collide({
            center: {
                x: me.center.x + ((me.facingRight ? 1 : -1) * 30),
                y: me.center.y,
            },
            size: {
                x: 30,
    ...
    function tickIntelligence(state) {
        if (state === 'ADVANCE') {
            ...
            var threatsAhead = mp.entities.collidingWith({
                center: me.center,
                size: { x: 50, y: 50 }
            }).filter(ent => ent.team && ent.team !== me.team)

            if (threatsAhead.length) {
                bot.push('["keyup", ' + LEFT +']\n')
            ...
            if (headbuttState.stage === 0) {
                if (Math.abs(headbuttState.target.x - me.center.x) < 12) {
                    bot.push('["keydown", ' + (heading() ? RIGHT : LEFT) + ']\n')
                    bot.push('["keydown", ' + JUMP + ']\n')
                    headbuttState.stage++
                }
            ...
            if (isUnderSights() && Math.random() > 0.8) {
                return m('KILL')
            }
        } else {
