...
module.exports = function makeMain({ networld, mp, isServer, debugInfo, drawQuadTree, debugInfoElement }) {
    ...
    var perfTrack =
        debugInfo && (typeof performance === 'object') &&
        (() => {
            ...
            return {
                start: (label) => {
                    assert(!(label in labels), 'Profiling for ' + label + ' already started!')
                ...
                frame: () => {
                    ...
                    eLabels.textContent = framesThisSecond + 'FPS,' + Object.keys(totals)
                        .map(total => total + '=' + roundTo(totals[total] / framesThisSecond, 2) + 'ms')
