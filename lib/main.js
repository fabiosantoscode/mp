...
module.exports = function makeMain({ networld, mp, isServer, debugInfo, drawQuadTree, debugInfoElement, clockSync }) {
    ...
    function draw(extrapolateAmount) {
        ...
        mp.ctx.clearRect(0, 0, mp.ctx.canvas.width, mp.ctx.canvas.height)
        ...
        var dynamicEnts = mp.entities.collidingWith({
            center: { x: offset.x + (mp.ctx.canvas.width / 2), y: mp.ctx.canvas.height / 2 },
            size: { x: mp.ctx.canvas.width, y: mp.ctx.canvas.height }
        }, { static: false })
