...
module.exports = function makeMain({ networld, mp, isServer }) {
    ...
    function createReadStream() {
        ...
        mp.entities.forEach( ent => {
            readStream.push(networld.createAddPacket(ent))
        })
    ...
    var _doRandomAddPacket = 0
    function randomAddPacket() {
        _doRandomAddPacket++
        if (_doRandomAddPacket % 40 !== 0) {
            return [];
        }
        var eligibleEnts = mp.entities.filter(ent => !ent.undying)
        if (!eligibleEnts.length) { return [] }
        return [networld.createAddPacket(
            eligibleEnts[Math.floor(Math.random() * eligibleEnts.length)])];
    }
