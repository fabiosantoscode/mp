...
function Entity(center) {
    ...
    this.solid = false
    this.speed = 1
    this.packetProps = ['clientsidePrediction', 'life', 'maxLife']  // Props which get serialized and updated
    this.life = -1
    this._changes = [];
}
...
Entity.prototype.remoteUpdate = function (packet) {
    ...
    for (var i = 0; i < this.packetProps.length; i++) if (name in packet) {
...
Object.defineProperty(Entity.prototype, 'moving', {
    ...
    set: function (newMoving) {
        if (!this._moving ||
            ...
            if (this.id) {
                this.pushChange('setMoving', newMoving.x, newMoving.y, this.center.x, this.center.y, this.direction.x, this.direction.y);
            }
        }
    }
});
...
Entity.prototype.update = function (stateless) {
    ...
    var { grounded, collidingWithMe } = this.collideWithThings()

    if (stateless !== true && (grounded || collidingWithMe)) {
        this.emit('bump', {collidingWithMe, grounded})
    }
...
Entity.prototype.collideWithThings = function () {
    ...
    var _collidingWithMe = entities.collidingWith(this, { solid: true })

    if ((computeGravity || computeMovementIntention) && this.direction.y >= 0) {
        var grounded = this.grounded(_collidingWithMe)
    }
    ...
    if (this.direction.y < 0 && !this.bumpless) {
        ...
        // Bump into ceilings
        for (var solid of _collidingWithMe) {
    ...
    return { grounded, collidingWithMe: _collidingWithMe }
}
...
Entity.prototype.groundedOnSolid = function (_collidingWithMe) {
    var highest = null
    _collidingWithMe = _collidingWithMe || entities.collidingWith(this, { solid: true })
    for (var solid of _collidingWithMe) {
...
Entity.prototype.jump = function (atX, atY) {
    ...
    this.pushChange('jump', this.center.x, this.center.y)
...
var packetsWhichRequireTimestamps = ['set3d', 'jump', 'setMoving']
...
Networld.prototype.commit = function () {
    ...
    return changedEnts.concat(diedEnts).concat(globalChanges)
        .map((change) => change.map(recursiveRound))
        .map((change) => {
            if (packetsWhichRequireTimestamps.indexOf(change[0]) !== -1)
                return [+new Date()].concat(change)
            return change
        })
}
