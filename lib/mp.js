...
Entity.prototype.setMoving = function (newMoving) {
    ...
    this._changes.push(['setMoving', this.id, newMoving]);
}
...
Entity.prototype.tryJump = function () {
    if (this.grounded()) {
        ...
        this._changes.push(['tryJump', this.id]);
    }
}
...
Player.prototype.createWriteStream = function () {
    ...
    ret._write = function onKeyData([type, keyCode], _, next) {
        if (type === 'keyup') {
            if (LEFT == keyCode) {
                if (wasMovingLeft) { player.setMoving(vec.origin) }
            }
            if (RIGHT == keyCode) {
                if (wasMovingRight) { player.setMoving(vec.origin) }
            }
            if (SHOOT == keyCode) {
                intendToShoot = true
            }
            ...
            if (LEFT == keyCode) {
                player.setMoving(vec({ x: -player.speed, y: 0 }));
                wasMovingLeft = true;
            }
            if (RIGHT == keyCode) {
                player.setMoving(vec({ x: player.speed, y: 0 }));
    ...
    var approach = function () {
        ...
        if (enemy.distanceTo(target) > minDistanceToPlayer && enemy.grounded()) {
            if (enemy.center.x >= target.center.x && facing !== -1) {
                facing = -1
                enemy.direction = vec({ x: -enemy.speed, y: 0 })
            } else if (enemy.center.x < target.center.x && facing != 1) {
                facing = 1
                enemy.direction = vec({ x: enemy.speed, y: 0 })
            }
        } else {
            enemy.direction = vec({ x: 0, y: enemy.direction.y })
        }
    }
...
Networld.prototype._onPacket = function (packet) {
    ...
    console.log('onPacket: ' +JSON.stringify(packet))
