...
function vec(arg) {
    assert(typeof arg.x === 'number' && typeof arg.y === 'number');
    assert(!isNaN(arg.x) && !isNaN(arg.y))
...
Object.defineProperty(Entity.prototype, 'center', {
    ...
    set: function (newCenter) {
        this.onUpdateCenter(this._center, newCenter)
...
Object.defineProperty(Entity.prototype, 'left', {
    ...
    set: function (left) {
        this.center = vect(left + this.halfWidth, this.center.y)
    }
});
...
Object.defineProperty(Entity.prototype, 'right', {
    ...
    set: function (right) {
        this.center = vect(right - this.halfWidth, this.center.y)
    }
});
...
Object.defineProperty(Entity.prototype, 'top', {
    ...
    set: function (top) {
        this.center = vect(this.center.x, top + this.halfHeight)
    }
});
...
Entity.prototype._pushChangeImpl = function (opt) {
    ...
    if (opt.timestamped)
        var toPush = [Date.now(), opt.method, this.id].concat(opt.args)
    else
...
Entity.prototype.logStatesAndInputs = function (specificFrameNumber) {
    ...
    thisState.input.intendToShoot = inputs.intendToShoot;
    thisState.input.intendToJump = inputs.intendToJump;
    thisState.input.intendToStopJump = inputs.intendToStopJump;
...
Bullet.prototype.update = function () {
    ...
    var collidingWithMe = entities.collidingWithNonStatic(this)

    for (var i = 0; i < collidingWithMe.length; i++) {
        if (ret.bulletExplodeFilter &&
                ret.bulletExplodeFilter({
                    owner: this.owner,
                    damaged: collidingWithMe[i] }) === false) {
            continue;
        }
        if (collidingWithMe[i] !== this.owner && collidingWithMe[i] !== this && collidingWithMe[i].owner !== this.owner && !collidingWithMe[i].ghost) {
            this.die(collidingWithMe[i])
            return
        }
    }
}
...
AmmoDrop.prototype.packetProps = Object.freeze(
    Entity.prototype.packetProps.concat(['count', 'class'])
)
