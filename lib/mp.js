...
Entity.prototype.draw = function (extrapolation) {
    ...
    if (this.smoothServerNudge) {
        var dt = +new Date()
        if (dt > this.smoothServerNudge.start + this.smoothServerNudge.duration) {
            this.smoothServerNudge = null
        } else {
            var currentAmount =
                (dt - this.smoothServerNudge.start) /
                this.smoothServerNudge.duration

            currentAmount = 1 - currentAmount

            var ix = Math.floor(this.smoothServerNudge.x * currentAmount)
            var iy = Math.floor(this.smoothServerNudge.y * currentAmount)
...
Explosion.prototype.extrapolatedSize = function (extrapolation) {
    return {
        x: ((this.lifetime + extrapolation) + 3) * 3,
        y: ((this.lifetime + extrapolation) + 3) * 3
    }
}
