...
Object.defineProperty(Entity.prototype, 'moving', {
    ...
    set: function (newMoving) {
        if ((this._moving > 0) !== (newMoving > 0)) {
            this.facingRight = (newMoving > 0);
...
Entity.prototype.applyServerCorrection = function (newCenter, newDirection) {
    ...
    var frame = packetAgeInFrames + 4
    var butterflyOffBy = Infinity
    while (frame--) {
        advanceMyCopy()
        if (frame < 8) {  // Getting there
            if (vec.distance(this.center, miniMe.center) < butterflyOffBy) {
                butterflyOffBy = vec.distance(this.center, miniMe.center)
