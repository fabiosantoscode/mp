...
EnemyAI.prototype.approach = function () {
    ...
    if (this.enemy.distanceTo(this.target) > this.minDistanceToPlayer && this.enemy.grounded()) {
        if (this.enemy.center.x >= this.target.center.x && this.facing !== -1) {
            this.facing = -1
            this.enemy.direction = vec({ x: -this.enemy.speed, y: 0 })
        } else if (this.enemy.center.x < this.target.center.x && this.facing != 1) {
            this.facing = 1
            this.enemy.direction = vec({ x: this.enemy.speed, y: 0 })
        }
    } else {
        this.enemy.direction = vec({ x: 0, y: this.enemy.direction.y })
    }
}
...
DogAI.prototype.approach = function () {
    ...
    if (dxToBiteTarget < 0) { this.enemy.direction = vec({ x: this.enemy.speed, y: this.enemy.direction.y }) }
    if (dxToBiteTarget > 0) { this.enemy.direction = vec({ x: -this.enemy.speed, y: this.enemy.direction.y }) }
}
...
Networld.prototype.commit = function () {
    ...
    var changedEnts = entities
        .filter(function didChange(ent) {
            var lastVer = this.lastVersions[ent.id]  // TODO this is a WeakMap
...
Networld.prototype._diff = function (version) {
    assert(this.isServer !== false, 'do not call _diff() on a client networld!')
...
Networld.prototype.makePacket = function (lastAckVersion) {
    assert(this.isServer !== false, 'do not call makePacket() on a client networld!')
    return {
        id: this.version,
