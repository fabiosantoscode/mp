...
Entity.prototype.draw = function (extrapolation) {
    ...
    var x1 = (x - (this.size.x >> 1) - camera.offset.x)
    var y1 = (y - (this.size.y >> 1))
    ...
    if (nudge) {
        ...
        if (dt > nudge.start + nudge.duration) {
            ...
            var ix = (nudge.x * nudge.currentAmount)
            var iy = (nudge.y * nudge.currentAmount)
...
Entity.prototype.update = function (stateless) {
    ...
    if (!this.solid && !this.bumpless && !this.groundless) {
        var res = this.collideWithThings()

        var grounded = res.grounded
        var collidingWithMe = res.collidingWithMe
    }
...
Explosion.prototype.findAffectedEntities = function () {
    ...
    return entities.collidingWith(area, { undying: false })
        .filter(ent => {
            assert(!ent.dead, 'TODO remove this after game runs for centuries without triggering this assertion')
            return !ent.dead
        })
}
