...
Entity.prototype.draw = function (extrapolation) {
    ...
    x1 += this.direction.x * extrapolation
    y1 += this.direction.y * extrapolation
...
Entity.prototype.update = function () {
    ...
    if (this.moving) {  // Some entities don't do this
        if (this.moving.x === 0 && this.direction.x !== 0) {
            ...
            this.direction.x *= this.grounded() ? 0.5 : 0.9;
...
Entity.prototype.tryJump = function () {
    if (this.grounded()) {
        this.direction.y -= (this.jumpSpeed || 4)
    }
}
...
Player.prototype.die = function () {
    if (Math.random() > 0.5) {
        ...
        drop.center = { x: this.center.x, y: this.center.y }
        entities.push(drop)
    }
...
Player.prototype.shoot = function () {
    var shootBullet = function (bullet) {
        bullet.center = { x: this.center.x, y: this.center.y }
...
Dog.prototype.update = function () {
    ...
    this.direction.x = this.biteTarget.direction.x
    this.direction.y = this.biteTarget.direction.y
}
...
Dog.prototype.unbite = function () {
    ...
    this.biting = false;
    this.biteStrengthLeft = -33;
    this.direction = { y: -3, x: (Math.random() - 0.5) * 5 }
}
...
Explosion.prototype.draw = function (extrapolation) {
    ...
    if (this.lifetime % 4 == 0) {  // TODO this comes from wall clock
        ...
        ctx.strokeRect(this.center.x - (size.x) - camera.offset.x,
            this.center.y - (size.y) - camera.offset.y,
            size.x * 2, size.y * 2)
        ctx.fillStyle = 'black'
    }
...
function AmmoDrop({ count, bullet }) {
    Entity.call(this)
    this.count = count
    this.bullet = bullet
...
EnemyAI.prototype.control = function (enemy) {
    ...
    enemy.onUpdate(function () {
        i++;
        if (i >= 16) { that.decideShit() }  // only decide shit every 16 ticks
    });
...
function DogAI() {
    ...
    this.biteXLocationInPlayer = (Math.random() - 0.5) * 10  // it tries to bite in this X coord
    ...
    this.target.onUpdate(function () {
        if (this.enemy.biting && !this.target.grounded()) {
            ...
            if (this.enemy.biteStrengthLeft < 33) {
                this.enemy.biteStrengthLeft++;
            }
        }
    }.bind(this))
