...
Entity.prototype.groundedOnSolid = function () {
    ...
    if (this._lastSolidGroundedOn) {
        if (this.collide(this._lastSolidGroundedOn)) {
    ...
    if (this.direction.y === 0) { return null }
...
Entity.prototype.bumpingIntoSolid = function () {
    var collidingWith = entities.collidingWith(this, { solid: true })
        .filter((solid) => {
            ...
            if (this.direction.x > 0 && solid.right - this.left < this.direction.x) return false
            if (this.direction.x < 0 && this.right  - solid.left < -this.direction.x) return false
            if (this.direction.x > 0 && this.right - this.direction.x > solid.left) { return false; }
            if (this.direction.x < 0 && this.left + -this.direction.x < solid.right) { return false; }
            return true
        })
...
AmmoDrop.prototype.draw = function () {
    ...
    this.center = vec({
        y: -(Math.sin((+new Date()) * (2 / 1000)) * 10) + this.center.y,
        x: this.center.x
    })
    Entity.prototype.draw.apply(this, arguments)
...
entities.on('remove', function (ent) { ent.emit('remove') })
